...
```[{"type": "text", "content": "What is the name of the country which flag is on the picture?"}, {"type": "image", "content": "images/00005.jpg"}]```

```[{"type": "text", "content": "What kind of bird makes such a sound?"}, {"type": "audio", "content": "audios/00005.wav"}]```

Изображения, по которым нужно сделать предсказания, хранятся в паке _images_. Внутри лежат файлы формата _00000.jpg_, _00001.jpg_, ...

Аудио файлы, по которым нужно сделать предсказания, хранятся в паке _audios_, в которой хранятся аудио файлы. Внутри лежат файлы формата _00000.wav_, _00001.wav_, ...
...
```
{
    "image": "cr.msk.sbercloud.ru/aijcontest2023_official/fusion:0.0.1",
...
* ```cr.msk.sbercloud.ru/aijcontest2023_official/fusion:0.0.1``` — Dockerfile и requirements для данного образа.
...
* ```generate_text(model: object, tokenizer: object, cur_query_list: List[Dict], history_list: List[Tuple[List[Dict], str]]) -> str```
    ...
    Функция, отвечающая за генерацию текстового ответа по входной реплике (**cur_query_list**) и истории диалога (**history_list**).
  
    В качестве аргументов функция принимает на вход объекты модели и токенизатора (возвращенные функцией `setup_model_and_tokenizer()`), текущую реплику – **cur_query_list** – в формате списка словарей, а также предыдущую историю диалога до текущей реплики – **history_list** – в формате списка кортежей, в которых нулевой элемент $i$-го кортежа — это реплика (список словарей), а первый элемент — ответ сгенерированный моделью в $i$-ом раунде диалога.
...
* ```get_ppl(model: object, tokenizer: object, cur_query_tuple: Tuple[List[Dict], str], history_list: List[Tuple[List[Dict], str]]) -> float```
    ...
    Функция, которая возвращает значение перплексии для входной реплики с некоторым текстом (cur_query_tuple) и историей текущего диалога (history_list).
...
Код участника не должен генерировать финальные файлы с ответами, ответы, сгенерированные с помощью функций `generate_text` и `get_ppl` собираются в финальные файлы в скрипте `run.py`.
...
Мы предоставляем участникам возможность получить доступ к вычислительным ресурсам Кристофари для обучения модели. Количество ресурсов ограничено. Для получения доступа необходимо отправить заявку на адрес Christofari_AIJContest_2023@sberbank.ru с описанием того, как именно планируется использовать вычислительные ресурсы.
...
## Специальная номинация "Что? Где? Когда?" 💎
...
в рамках которой разработанные Участниками мультимодальные модели смогут принять участие в импровизированной игре «Что? Где? Когда?» 
и ответить на 10 сложных и интересных вопросов, подобных тем, что задаются знатокам в одноименной интеллектуальной игре.
...
Модели нужно ответить на 10 вопросов в текстовом виде. Каждый текстовый ответ оценивается Комиссией, в которую входят 
